...
| <a name="input_everybody_else_limit"></a> [everybody\_else\_limit](#input\_everybody\_else\_limit) | Rate limit for all country\_codes not covered by country\_rates. Set limit to 0 to disable the rule. | <pre>object({<br/>    limit    = optional(number, 0)<br/>    priority = optional(number, 80)<br/>  })</pre> | `{}` | no |
